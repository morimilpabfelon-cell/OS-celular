...
case "$EXPECTED_SHA256" in
    *[!0-9A-Fa-f]*|'') fail 'ARCH_ROOTFS_EXPECTED_SHA256 must be a 64-character hexadecimal digest' ;;
esac
...
for command_name in curl gpg sha256sum bsdtar python3 mktemp awk tr id; do
...
ACTUAL_FINGERPRINT=$(gpg --homedir "$GNUPGHOME" --batch --with-colons --fingerprint "$SIGNING_FINGERPRINT" |
...
ACTUAL_SHA256=$(sha256sum "$ARCHIVE" | awk '{ print $1 }' | tr 'A-F' 'a-f')
